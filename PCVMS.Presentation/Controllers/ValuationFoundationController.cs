...
{
    [Authorize]
    public class ValuationFoundationController : Controller
    {
        ...
        public ValuationFoundationController(IMemoryCache _Cache, IOptions<PCVM.Web.Helpers.AppSettings> appSettings)
        {
        ...
        public async Task<List<PCVMS_Resource>> GetAllResource()
        {
            try
            {
                ...
                using (var objectClient = new ApiClient(AppSettings))
                {
                    ...
                    {
                        Model.Resource.Add(a.Key, a.Value);
