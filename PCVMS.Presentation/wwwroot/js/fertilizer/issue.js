...
$(document).ready(function () {
    ...
    $("#tbl-fertilizer-certificate-list > tbody").on("click", "tr td i", function (evt) {
       ...
       //end
        ...
        if (event === 'pdf certificate' || event === 'print certificate' || event === 'view certificate') {

            alert('pdf certificate');

            generatepdf("local", id);

        } else if (event === 'print certificate') {

            alert('print certificate');

            $("#receipt_details :input").attr("disabled", true);
            $("#SaveReceiptTransaction").prop("disabled", true);

        } else if (event === 'view certificate') {

            alert('view certificate');

            $("#receipt_details :input").attr("disabled", false);
            $("#SaveReceiptTransaction").prop("disabled", false);
...
function getAllIssuedCertificate() {

    $.ajax(
        {
            type: "GET", //HTTP POST Method
            url: "getAllIssuedCertificate", // Controller/View
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var tempString = "<tr>"
           ...
           + "<td id ='tdstatus' name='tdstatus' style='text-align:center;'>" + data[i].statusId + "</td>"
            + "<td class='text-center' id='tdAction' name='tdAction'><div class='position-relative'><i class='metismenu-icon lnr-license' style='cursor: pointer; margin-left: 10px;' title='view certificate'></i><i class='metismenu-icon lnr-printer' style='cursor: pointer; margin-left: 10px;' title='print certificate'></i><i class='metismenu-icon lnr-book' style='cursor: pointer; margin-left: 10px;' title='pdf certificate'></i></div></td></tr>";
          // addpend the final string
          $("#tbl-fertilizer-certificate-list > tbody:last-child").append(tempString);
    }
}

function generatepdf(application_Type, id) {

   $.ajax(
        {
            type: "GET", //HTTP POST Method
           url: "CertificateDesign?application_type=" + application_Type +"&id=" + id, // Controller/View
