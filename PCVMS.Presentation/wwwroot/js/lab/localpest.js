...
//# Region GENERAL PAGE
    ...
    $("#btnSaveImportExport").click(function (e) {
        ...
        var App_ID;
        var App_Name;
        if (e.target.name === 'LocalAgriculture') {

            App_ID = 'A757ADAA-7436-4F7E-91C9-548E19E52AE4';
            App_Name = 'Local-Agriculture';
        } else if (e.target.name === 'LocalPesticide') {

            App_ID = '30621807-B943-4E3D-A76F-9CE8320A4A87';
            App_Name = 'Local-Pesticide';
        }
        else if (e.target.name === 'LocalPest') {

            App_ID = '19968538-8B2A-45C7-BC67-6E1F55ABB9F9';
            App_Name = 'Local-Pest';
        } else if (e.target.name === 'LocalFertilizerSoil') {

            App_ID = '0C82B314-D0EC-4C3E-BF8A-8ED1FEA7E93A';
            App_Name = 'Local-Fertilizer-Soil';
        }
        ...
        $.ajax(
            {
                ...
                data: { //Passing data
                    ...
                    App_ID: App_ID,
                    App_Name: App_Name,
                    Governorate_ID: $("#GovernorateID").val(),
                    Governorate: $("#GovernorateID option:selected").text(),
                    Village: $("#txtVillage").val(),
                    AgriculturalHoldingNumber: $("#txtAgriculturalHoldingNumber").val(),
                    wgs84_latitude: $("#txtlatitude").val(),
                    wgs84_longitude: $("#txtlongitude").val(),
                    ...
                    Farm_Type_ID: $("#FarmTypeID").val(),
                    Farm_Type: $("#FarmTypeID option:selected").text(),
                    Farm_Owner_Name: $("#txtFarmerName").val(),
        ...
        $.ajax(
            {
                ...
                data: { //Passing data
                    ...
                    ID: 0,
                    Sample_ID: $("#ID").val(),
                    Expected_Harvest_Date: $("#txtDateofSymptoms").val(), //Reading text box values using Jquery
                    Production_Date: $("#txtDateofSymptoms").val(),
                    Expiry_Date: $("#txtDateofSymptoms").val(),

                    Plant_Type_ID: $("#PlantMasterID").val(),
                    Plant_Type: $("#PlantMasterID option:selected").text(),
                    Plant_Name: $("#txtPlantName").val(),                   
                    Area_Quantity_Estimated: $("#txtArea").val(),

                    Infected_Part_Plant_ID: $("#InfectedPlantPartsID").val(),
                    Infected_Part_Plant_Name: $("#InfectedPlantPartsID option:selected").text(),
                    Cultivation_Type_ID: $("#CultivationTypeMasterID").val(),
                    Cultivation_Type_Name: $("#CultivationTypeMasterID option:selected").text(),
                    Irrigation_System_ID: $("#IrrigationSystemMasterID").val(),
                    Irrigation_System_Name: $("#IrrigationSystemMasterID option:selected").text(),

                    Infection_Symptoms_ID: $("#InfectionSymptomsMasterID").val(),
                    Infection_Symptoms_Name: $("#InfectionSymptomsMasterID option:selected").text(),

                    Percentage_Infection_One_Plant_ID: $("#InfectionInOnePlantID").val(),
                    Percentage_Infection_One_Plant_Name: $("#InfectionInOnePlantID option:selected").text(),

                    Percentage_Infection_In_Field_ID: $("#InfectionInFieldID").val(),
                    Percentage_Infection_In_Field_Name: $("#InfectionInFieldID option:selected").text(),
                    Date_Symptoms_On_Set: $("#txtDateofSymptoms").val(),
                    Age_Plant_Growth_Stage: $("#txtPlantAge").val(),
                    Symptoms_Description: $("#txtSymptomsDescription").val(),
                ...
                success: function (data) {
                    ...
                    // Populate the Table with Sample List
                    getSampleDetailsByID($("#ID").val(), "Saved");
        ...
        $.ajax(
            {
                ...
                data: { //Passing data
                    ...
                    Pesticide_Type_ID: $("#PesticideTypeMasterID").val(),
                    Pesticide_Type_Name: $("#PesticideTypeMasterID option:selected").text(),
                    Remark: $("#txtRemark").val(),
                    StatusId: "Saved"
...
//# end Region GENERAL PAGE
    ...
    $("#tbl-lab-sample-list > tbody").on("click", "tr td i", function (evt) {
        ...
        if (event === 'view') {
            ...
            $("#step-1 *").attr("disabled", true);
            $("#step-2 *").attr("disabled", true);
        ...
        $("#ID").val($(this).parents("tr").find("#tdid").html());
        ...
        getSampleDetailsByID($("#ID").val(), status);
        GetSampleAnalysisByID($("#ID").val(), status);
        GetSampleResultByID($("#ID").val(), status);
        populateDocument($("#ID").val());
        populateRFCDetail();
    });
    ...
    // Datepicker for Pesticide Certificate
    $('[data-toggle="datepicker_trigger_txtDateofSymptoms"]').datepicker({
        trigger: ".datepicker-click-txtDateofSymptoms",
        autoPick: true,
        autoHide: true
    });
...
function GetSampleAnalysisByID(id, status) {
    ...
    $.ajax(
        {
            type: "GET", //HTTP POST Method
            url: "GetSampleAnalysisByID?ID=" + $("#ID").val(),
            success: function (data) {
...
function GetSampleResultByID(ID, status) {
    ...
    $.ajax(
        {
            type: "GET", //HTTP POST Method
            url: "GetSampleResultByID?ID=" + $("#ID").val(),
            success: function (data) {
...
function populateLabSampleDetailsGrid(data, status) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        var tempString = "<tr>"
            + "<td id = 'tdid' name='tdid'>" + data[i].plant_Type + "</td>"
            + "<td id = 'tdCommodity_Type_Name' name = 'tdCommodity_Type_Name' style = 'text-align:center;'>" + data[i].plant_Name + "</td>"
            + "<td id = 'tdCommodity_Name' name = 'tdCommodity_Name' style = 'text-align:center;'>" + data[i].area_Quantity_Estimated + "</td>"
