...
namespace PCVM.MsSql.Implementation
{
    public class DocumentRepository : IDocumentRepository
    {
        ...
        public async Task<bool> ChangeState(Guid id, string nextState,  string nextStateName)
        {
        ...
        public List<PCVMS.Model.BusinessModel.Document> Get(out int count, int page = 0, int pageSize = 128)
        {
            ...
            return query.Include(x => x.Author)
                        ...
                        .Take(pageSize)
                        .ToList()
                        .Select(d => Mappings.Mapper.Map<PCVMS.Model.BusinessModel.Document>(d)).ToList();
        }
        ...
        public List<PCVMS.Model.BusinessModel.DocumentTransitionHistory> GetHistory(Guid id)
        {
            ...
            return _sampleContext.DocumentTransitionHistories
                 ...
                 .ThenBy(h => h.Order)
                 .ToList()
                 .Select(x => Mappings.Mapper.Map<PCVMS.Model.BusinessModel.DocumentTransitionHistory>(x)).ToList();
        }

        public List<PCVMS.Model.BusinessModel.Document> GetInbox(Guid identityId, out int count, int page = 0, int pageSize = 128)
        {
        ...
        public List<PCVMS.Model.BusinessModel.Document> GetOutbox(Guid identityId, out int count, int page = 0, int pageSize = 128)
        {
            int actual = page * pageSize;
        ...
        public PCVMS.Model.BusinessModel.Document InsertOrUpdate(PCVMS.Model.BusinessModel.Document doc)
        {
        ...
        public PCVMS.Model.BusinessModel.Document Get(Guid id, bool loadChildEntities = true)
        {
            Document document = GetDocument(id, loadChildEntities);
            if (document == null) return null;
            return Mappings.Mapper.Map<PCVMS.Model.BusinessModel.Document>(document);
        }
